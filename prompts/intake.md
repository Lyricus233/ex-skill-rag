...
# 步骤 2：告知 RAG 能力
告诉用户，在这个 skill 生成过程中或者完成之后，我们会采用基于 Milvus 的 RAG 向量检索来随时调用历史原材料（比如具体的聊天片段、事件）。请用户放心，如果资料较多，RAG 功能也可以确保长期记忆不会遗漏。对于可能存在的不准确的细节，只要用户补充新的聊天记录，即可无缝继续采用 `ingest_milvus.py` 追加数据（无需覆盖原有知识），即可自动进行长期融合。

最后汇总以上内容并准备进入原材料收集流程。
