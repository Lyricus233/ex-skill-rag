...
2. **【AgentSkills RAG 要求】** 在组装和提炼大段记忆前，对不确定的具体名称、具体争吵原因，强烈建议采用 `search_milvus.py` 对对应 collection 执行关键词检索以获得原音重现，而不是单纯依赖上下文缓存。
3. 时间信息尽量精确（从消息时间戳推断）
4. 地点信息可从照片 EXIF 或聊天内容中提取
5. 争吵和甜蜜记忆同等重要——都是真实的ta
6. 分手相关内容要客观，不做任何方的美化或丑化
7. 如果信息不足，标注 `[待补充]` 而非推测
