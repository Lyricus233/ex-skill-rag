...
def rollback(base_dir: str, slug: str, version: str):
    ...
    print(f"\n[⚠️ 提示]: 本次回滚仅恢复了人物设定与总结层（Persona & Memory）。")
    print(
        f"如果您是因为『错误追加了聊天记录』而回滚，Milvus 向量库 (ex_{slug}_memories) 中的原始切片依然保留。"
    )
    print(
        f"如果需要剔除错误向量数据，建议通过查询特定的 chat_id / source 手动从 Milvus 删除。"
    )
